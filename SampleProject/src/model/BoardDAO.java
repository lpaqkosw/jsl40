...
public class BoardDAO {
	...
	public List<BoardVO> boardSearchList(int startlist, int endlist, String item, String key) {
		String sql = "select A.* from (select rownum as rn,B.* from (select * from board where "+item+" like ? order by regdate desc) B) A where rn > ? and rn <= ? ";
		List<BoardVO> list= new ArrayList<BoardVO>();
		try {
			conn = manager.getConnection();
			ps = conn.prepareStatement(sql);
	...
	public List<BoardVO> indexBoardList(int bcnt) {

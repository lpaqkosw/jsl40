...
<script>
	function send(){
		...
		if(form_name.userid.value == "" || form_name.userid.value.length < 5 || form_name.userid.value.length > 16){
			alert("Please enter a valid Id");
			form_name.userid.focus();
			return;
		}
		
		if(form_name.passwd.value.length < 6 || form_name.passwd.value.length > 12){
			alert("Please enter a valid password (6~12 characters)");
			form_name.passwd.focus();
			return;
		}
		if(form_name.passwd.value != form_name.repasswd.value){
			alert("Password not same");
			form_name.repasswd.focus();
			return;
		}
		for(y = 0,flag1=0;y<form_name.gubun.length;y++){
			if(form_name.gubun[y].checked){
				flag1++;
				break;
			}
		}
		if(flag1==0){
			alert("Please select gubun");
			return;
		}
		if(form_name.tel.value == ""){
		...
		var i=0;
		var flag=0;
		for(i=0;i<form_name.fa.length;i++){
			if(form_name.fa[i].checked){
				flag++;
				break;
			}
		}
		if(flag==0){
			alert("check at least one fa");
			return;
		}
